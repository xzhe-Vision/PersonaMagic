...
class MyAttenProc:
    def __call__(self, attn: CrossAttention,
        ...
        if encoder_hidden_states is not None:
            if isinstance(encoder_hidden_states, dict):
                _ehs = encoder_hidden_states[f"CONTEXT_TENSOR"]
                if f"CONTEXT_TENSOR_BYPASS" in encoder_hidden_states:
                    _ehs_bypass = encoder_hidden_states[f"CONTEXT_TENSOR_BYPASS"]
            else:
                _ehs = encoder_hidden_states
        else:
            _ehs = None
