...
def sd_pipeline_call(
        ...
        noise_latents: Optional[torch.FloatTensor] = None):
...
def invert(
    pipeline: StableDiffusionPipeline,
    prompt_embeds: torch.FloatTensor,
    start_latents: torch.FloatTensor,
    num_inference_steps: int = 50,
    guidance_scale: float = 8.0,
    negative_prompt: Optional[Union[str, List[str]]] = None,
    num_images_per_prompt: Optional[int] = 1):
    ...
    for i, t in enumerate(timesteps):
        ...
        embed = prompt_embeds[0] if type(prompt_embeds) == list else prompt_embeds
        ...
        noise_pred_text = noise_pred_uncond + guidance_scale * (noise_pred_text - noise_pred_uncond)
        ...
        latents = (latents - (1-alpha_t).sqrt()*noise_pred_text)*(alpha_t_next.sqrt()/alpha_t.sqrt()) + (1-alpha_t_next).sqrt()*noise_pred_text
